...
Uses:
  - modules.detector.VehicleDetector
  - modules.helmet.HelmetChecker
  - modules.anpr.ANPRReader

Run (either works):
  cd /path/to/visiongate_x && python modules/live_campus_caption.py
  cd /path/to/visiongate_x && python live_campus_caption.py   # copy at repo root
"""
...
# Project root = parent of this `modules/` folder
...
SHOW_WINDOW = True
# Console: natural sentence when the line changes (not every N frames).
PRINT_LIVE_CAPTIONS = True
# Optional: raw debug (plate / helmet scores) — off by default.
VERBOSE_DEBUG = False
PRINT_EVERY_N_FRAMES = 15  # only used when VERBOSE_DEBUG is True
USE_SQLITE = True
SQLITE_DB = os.path.join(_ROOT, "outputs", "campus_events.db")

# Gate session (bike in frame): wait, then commit one caption without waiting for the bike to leave.
CAPTION_COMMIT_MIN_SEC = 0.75
CAPTION_COMMIT_MAX_SEC = 2.0
PLATE_SAMPLES_BEFORE_COMMIT = 5
# No bike for this many frames ⇒ reset session for the next vehicle.
...
def _helmet_with_without(display: str) -> str:
    """Short phrase after number plate: 'with a helmet' / 'without a helmet'."""
...
def build_gate_caption(
    *,
    ...
    """Single readable line for overlay + console, e.g. gate entry narrative."""
    if not has_bike:
